...
# 
logger_jf = logging.getLogger()

logHandler = logging.StreamHandler()
#logHandler = logging.FileHandler(filename='/logs/app/my-log.json')
logHandler.setFormatter(jsonlogger.JsonFormatter())
logger_jf.addHandler(logHandler)
logger_jf.setLevel(logging.DEBUG)
...
# @capture_span()
# @runtime_logging
@tracer.wrap(name='do_something')
def do_something(num):
    return {"do": f"something on {service}", "factorial_comp": something_important(num)}

@tracer.wrap(name='do_something_else')
def do_something_else(num):
    return {"do": f"something else on {service}", "factorial_comp": something_important(num)}

# @capture_span()
@tracer.wrap(name='something_important')
def something_important(num):
    return fact_loop(num) == fact_recursion(num)

# @capture_span()
@tracer.wrap(name='fact_recursion')
...
# @capture_span()
@tracer.wrap(name='fact_loop')
def fact_loop(num):
    # Use the .trace method to create 
    # a span named "my_trace" to trace
    # just these lines of code:
    with tracer.trace("fact_loop_trace") as span:
        # Designate this span as a separate service:
        span.service="fact_loop_service"
        # Add metadata:
        span.set_tag('service_type','fact_loop')
        # 
        span.set_tag('service_parameter', num)
